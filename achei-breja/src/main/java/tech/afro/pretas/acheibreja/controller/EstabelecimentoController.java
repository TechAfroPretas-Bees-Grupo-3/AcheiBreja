...
public class EstabelecimentoController {
	...
	@PostMapping("/criar") // criar paramentro com classe para receber o json
	...
	public ResponseEntity<Estabelecimento> update(@Valid @RequestBody Estabelecimento estabelecimentoParaAtualizar) {
		...
		Optional<Estabelecimento> estabelecimentoBuscado = repository
				.findById(estabelecimentoParaAtualizar.getIdEstabelecimento());

		if (estabelecimentoBuscado.isPresent()) {

			repository.save(estabelecimentoParaAtualizar);

			return ResponseEntity.status(HttpStatus.ACCEPTED).body(estabelecimentoParaAtualizar);
		} else {
